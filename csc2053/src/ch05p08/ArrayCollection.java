...
public class ArrayCollection<T> implements CollectionInterface<T>  
{
  ...
  public String toString() {
	  //Returns a String object representing this array collection.
	  //5.8a
	  String str = "[";
	  
	  int location;
	  for (location = 0; location < numElements-1; location++) {
		  str += (elements[location] + ", ");
	  }
  ...
  public int count(T target) {
	  ...
	  for (int location = 0; location < numElements; location++) {
		  if (elements[location].equals(target)) {
			  count++;
		  }
	  }
  ...
  public void removeAll(T target) {
	  //Removes all of the elements that match target such that for element e e.equals(target)
	  //return true. Each empty spot is replaced in order by the current last element.
	  //5.8c
	  for (int location = 0; location < numElements; location++) {
		  if (elements[location].equals(target)) {
			  elements[location] = elements[numElements-1];
